...
@testset "oclust" begin
    n_rows = 10
    n_cols = 20
    raw_data = rand(10, 20)
    raw_moment = compute_moment(raw_data)

    rows_order, cols_order = slanted_orders(raw_data)
    @assert length(rows_order) == n_rows
    @assert length(cols_order) == n_cols

    slanted_data = raw_data[rows_order, cols_order]
    slanted_moment = compute_moment(slanted_data)
    ...
    rows_rclust = reorder_hclust(rows_hclust, rows_order)
    cols_rclust = reorder_hclust(cols_hclust, cols_order)

    rclust_data = raw_data[rows_rclust.order, cols_rclust.order]
    rclust_moment = compute_moment(rclust_data)
